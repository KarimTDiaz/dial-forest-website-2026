    ...
    return (
        <div className='flex min-h-svh flex-col'>
            <header className=' sticky top-0 z-50 flex items-center justify-between p-3 md:p-4'>
                ...
                >
                    <span
                        className={`block h-0.5 w-6 rounded-sm bg-text-heading transition-all duration-300 ${menuOpen ? 'translate-y-[7px] rotate-45' : ''}`}
                    />
                    <span
                        className={`block h-0.5 w-6 rounded-sm bg-text-heading transition-all duration-300 ${menuOpen ? 'opacity-0' : ''}`}
                    />
                    <span
                        className={`block h-0.5 w-6 rounded-sm bg-text-heading transition-all duration-300 ${menuOpen ? '-translate-y-[7px] -rotate-45' : ''}`}
                    />
            ...
            <a
                ...
                className='fixed z-50 flex h-17 w-17 items-center justify-center rounded-full bg-text-heading px-2 text-center text-[0.65rem] font-black uppercase leading-tight tracking-wide text-brand transition-transform hover:scale-105 active:scale-95 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-text-heading md:h-18 md:w-18 md:text-xs'
                style={{
                    bottom: 'max(1rem, env(safe-area-inset-bottom, 0px))',
                    right: 'max(1rem, env(safe-area-inset-right, 0px))',
                }}
                aria-label='Comprar entradas en Ingresse (se abre en una pestaña nueva)'
            >
                Tickets
            </a>
